...
import { DealroomStatus } from '@prisma/client'
...
const ACTIVE_STATUSES: DealroomStatus[] = [
  DealroomStatus.DISCOVERY,
  DealroomStatus.NDA_SIGNED,
  DealroomStatus.INITIAL_REVIEW,
  DealroomStatus.DUE_DILIGENCE,
  DealroomStatus.NEGOTIATION,
  DealroomStatus.LOI_PENDING,
  DealroomStatus.LOI_SIGNED,
  DealroomStatus.UNDER_CONTRACT
]
  ...
  try {
    ...
    const dealrooms = await prisma.dealroom.findMany({
      ...
      select: {
        id: true,
        updatedAt: true,
        lastActivityEmailSentAt: true,
        inactivityEmailCount: true,
        buyer: {
          select: {
            id: true,
            email: true,
