...
- **Context Engineering & AI Tooling Integration:**
  - **Vendored AI Tools:** Leverage tools in `vendor/` for enhanced development workflows:
    - **claude-mem** (`vendor/claude-mem/`): Persistent memory compression for Claude Code sessions
    - **Agent Skills for Context Engineering** (`vendor/agent-skills-for-context-engineering/`): Context engineering principles
  - **Context Engineering Skills:** Apply principles from `vendor/agent-skills-for-context-engineering/skills/`:
  ...
  - **Cursor Integration:** Reference `.cursor/rules/ai-tooling.mdc` for SellerFi-specific workflows and prompt templates
  - **Documentation:** See `docs/AI_TOOLING.md` for complete setup and usage instructions
...
9. **Skills-Enhanced Strategy:** Always consider available skills from the `.claude/skills/` directory that can augment agent capabilities and improve project outcomes.
